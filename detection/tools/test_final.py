...
def main():
    ...
    for epoch_num in range(args.start_epoch, args.end_epoch + 1):
        if args.weight_file:
            ...
            weight_file = "workdirs/log-of-{}/epoch_{}.pkl".format(
        ...
        if args.weight_file:
            ...
            json_path = "workdirs/log-of-{}/test_epoch_{}.json".format(
                os.path.basename(args.file).split(".")[0], epoch_num
            )
...
def build_dataloader(dataset_dir, cfg):
    val_dataset = data_mapper[cfg.test_dataset["name"]](
        ...
        os.path.join(dataset_dir, cfg.test_dataset["name"], cfg.test_dataset["test_ann_file"]),
        order=["image", "info"],
    )
