...
from tools.transforms import RandomSizedBBoxSafeCrop
...
def main():
    ...
    # log_dir = "workdirs/log-of-{}-{}".format(os.path.basename(args.file).split(".")[0], time.strftime("%Y-%m-%d-%H-%M", time.localtime()))
    log_dir = "workdirs/log-of-{}".format(os.path.basename(args.file).split(".")[0])
    if not os.path.isdir(log_dir):
        os.makedirs(log_dir)
    mge.set_log_file(os.path.join(log_dir, "log.txt"))
...
def worker(args):
    ...
    if args.weight_file is not None:
        logger.info("Loading COCO pretrained...")
    ...
    for epoch in range(model.cfg.max_epoch):
        ...
        if dist.get_rank() == 0:
            save_path = "workdirs/log-of-{}/epoch_{}.pkl".format(
...
def build_dataloader(batch_size, dataset_dir, cfg):
    ...
    train_dataloader = DataLoader(
        ...
        transform=T.Compose(
            transforms=[
                # RandomSizedBBoxSafeCrop(height=1024, width=1024, p=1),
